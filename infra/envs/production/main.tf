terraform {
  cloud {
    organization = "onye" # replace with your Terraform Cloud org name
    workspaces {
      name = "onye-radiology-production"
    }
  }
...
module "b2" {
  ...
  bucket_name     = "onye-dicom-production"
  key_name        = "onye-tus-production"
...
module "tus_server" {
  ...
  service_name  = "onye-tus-production"
  environment   = "production"
  plan          = "standard"
  region        = "oregon"
